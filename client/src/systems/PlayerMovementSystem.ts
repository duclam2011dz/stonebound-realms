...
import { PLAYER_AIR_MOVE_SPEED_FACTOR, type GameSettings } from '../config/constants';
import {
  applyAcceleration,
  applyAirDrag,
  applyGroundFriction,
...
export class PlayerMovementSystem {
  ...
  updateSurvival(transform: TransformComponent, physics: PhysicsComponent, dt: number): void {
    ...
    const movingForward = this.input.isKeyDown('KeyW');
    const movingBackward = this.input.isKeyDown('KeyS');
    const movingSideways = this.input.isKeyDown('KeyA') || this.input.isKeyDown('KeyD');
    const slowMove = !movingForward && (movingBackward || movingSideways);
    const groundWishSpeed = physics.moveSpeed * (slowMove ? 0.5 : 1);
    const airWishSpeed = groundWishSpeed * PLAYER_AIR_MOVE_SPEED_FACTOR;
    const horizontalSpeedLimit = physics.onGround
      ? physics.moveSpeed
      : physics.moveSpeed * PLAYER_AIR_MOVE_SPEED_FACTOR;
    ...
    this.clampHorizontalSpeed(physics.velocity, horizontalSpeedLimit);
