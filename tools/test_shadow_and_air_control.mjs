...
const runCommand = async (command) => page.evaluate((text) => window.execute_game_command?.(text), command);
const advance = async (ms) => page.evaluate((deltaMs) => window.advanceTime?.(deltaMs), ms);

await runCommand("/time set day");
await advance(1000);

const setShadowScenario = async (mode) => page.evaluate((scenarioMode) => {
  ...
  const sunInfo = world.getSunOcclusionAt(player.x, player.y + 1.6, player.z, dayNight.sunDirection);
  return {
    scenarioMode,
    sunInfo,
...
for (const scenario of scenarios) {
  ...
  await advance(320);
  shadowMetrics[scenario] = await setShadowScenario(scenario);
  await page.screenshot({ path: path.join(outDir, `shadow-${scenario}.png`), omitBackground: false });
}
