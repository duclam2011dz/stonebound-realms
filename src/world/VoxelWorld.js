...
import { CHUNK_SIZE, DEFAULT_SETTINGS, PLAYER_HEIGHT, PLAYER_RADIUS, WORLD_MAX_HEIGHT } from "../config/constants.js";
import { getProceduralAtlasAssets } from "../textures/proceduralBlockAtlas.js";
import { createVoxelMaterial } from "../core/render/lighting/createVoxelMaterial.js";
import { TerrainGenerator } from "./services/TerrainGenerator.js";
import { VoxelChunkMesher } from "./services/VoxelChunkMesher.js";
import { VoxelRaycaster } from "./services/VoxelRaycaster.js";
import { VoxelStorage } from "./services/VoxelStorage.js";
import { ChunkTaskQueue } from "./services/chunks/ChunkTaskQueue.js";
import { planVisibleChunks } from "./services/chunks/planVisibleChunks.js";
import { BLOCK_ID_LAMP } from "./services/BlockPalette.js";
...
export class VoxelWorld {
  constructor(scene, settings, worldConfig = {}) {
    ...
    this.mesher = new VoxelChunkMesher(this.chunkSize, this.maxHeight, (cx, cz) => this.getChunkLightSources(cx, cz));
  ...
  getChunkLodStep(cx, cz) {
    const ringDistance = Math.max(Math.abs(cx - this.currentChunkX), Math.abs(cz - this.currentChunkZ));
  ...
  updateVisibleChunksAround(position, force = false) {
    ...
    const chunkChanged = !this.hasChunkCenter || pxChunk !== this.currentChunkX || pzChunk !== this.currentChunkZ;
    ...
    this.visibleEpoch += 1;
    const desiredPlan = planVisibleChunks(pxChunk, pzChunk, this.renderDistance, (cx, cz) => this.storage.chunkKey(cx, cz));
  ...
  raycastFromCamera(camera, maxDistance) {
    return this.raycaster.raycast(camera, maxDistance, (x, y, z) => this.storage.isBlockFilled(x, y, z));
  }
  ...
  getSunOcclusionAt(x, y, z, sunDirection) {
    ...
    for (const yOffset of frontRayOffsets) {
      const trace = this.traceFilledRay(
        x,
        y + yOffset,
        z,
        dirX,
    ...
    const frontShadow = frontBlocked
      ? clamp(
        frontBlockedFraction * (0.34 + frontHeightFactor * 0.38 + lowSunBoost * 0.34),
        0,
        1
      )
      : 0;
    ...
    structuralShadow = Math.max(structuralShadow, directionalShadow * (overheadBlocked ? 0.82 : 0.34));
    ...
    const ambientVisibility = caveFactor > 0.96
      ? 0.04
      : clamp(0.22 + skyVisibilityBlend * 0.78, 0.04, 1);
