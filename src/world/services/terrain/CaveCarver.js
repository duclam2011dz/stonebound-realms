import { BLOCK_ID_AIR } from "../BlockPalette.js";
import { coordHash } from "./terrainHash.js";
...
export class CaveCarver {
  ...
  carveRegionWorms(storage, regionX, regionZ, bounds, getSurfaceY) {
    ...
    for (let i = 0; i < wormCount; i++) {
      ...
      this.carveWorm(storage, startX, startY, startZ, yaw, pitch, segmentCount, baseRadius, rng, bounds, getSurfaceY);
    }
  }

  carveWorm(storage, startX, startY, startZ, startYaw, startPitch, segmentCount, baseRadius, rng, bounds, getSurfaceY) {
    let x = startX;
    let y = startY;
    let z = startZ;
    let yaw = startYaw;
  ...
  carveEllipsoid(storage, centerX, centerY, centerZ, radiusX, radiusY, radiusZ, bounds, getSurfaceY) {
