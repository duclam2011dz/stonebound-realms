import { BIOME_DESERT, BIOME_FOREST, BIOME_HILL, BIOME_PLAIN } from "./biomeTypes.js";
...
export class TerrainHeightModel {
  ...
  getHeight(x, z, biomeId = this.biomeModel.getBiomeId(x, z)) {
    ...
    if (biomeId === BIOME_PLAIN) {
      ...
      const rolling = this.noise.fractalSimplex2D(x * 0.0063 + 320, z * 0.0063 - 260, 4, 0.56, 2.12);
      baseHeight = 34 + continental * 10 + rolling * 7 + detail * 2.5 + erosion * 2.6;
    } else if (biomeId === BIOME_HILL) {
      const ridgeSource = this.noise.fractalSimplex2D(x * 0.0032 + 210, z * 0.0032 + 210, 5, 0.5, 2.05);
      const ridge = 1 - Math.abs(ridgeSource);
      const ridgeBoost = ridge * ridge;
      const mountainMaskNoise = this.noise.fractalPerlin2D(x * 0.0011 - 420, z * 0.0011 + 300, 3, 0.5, 2);
      ...
      const longWaves = this.noise.fractalPerlin2D(x * 0.0023 - 450, z * 0.0023 + 390, 3, 0.52, 2.05);
      const duneRidges = Math.abs(dunes) * 6.8;
      baseHeight = 28 + continental * 7 + longWaves * 4.5 + duneRidges + detail * 1.8 + erosion * 1.4;
    }
  ...
  buildHeightMap(baseX, baseZ, chunkSize, biomeMap = null) {
    ...
    for (let lz = 0; lz < chunkSize; lz++) {
      for (let lx = 0; lx < chunkSize; lx++) {
        ...
        const biomeId = biomeMap ? biomeMap[lx + lz * chunkSize] : this.biomeModel.getBiomeId(worldX, worldZ);
