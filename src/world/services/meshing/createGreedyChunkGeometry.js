...
function computeSkyLight(heights, baseX, baseZ, chunkSize, maxHeight, storage, worldX, worldY, worldZ) {
  const skyHeight = getSkyHeightAt(heights, baseX, baseZ, chunkSize, maxHeight, storage, worldX, worldZ);
...
function fillChunkCells(cells, storage, cx, cz, lodStep, chunkSize, maxHeight, sizeX, sizeY, sizeZ) {
  if (lodStep === 1) {
...
export function createGreedyChunkGeometry({
  ...
  const getCell = (x, y, z) => {
    ...
    return sampleLodCell(storage, baseX + x * lodStep, y * lodStep, baseZ + z * lodStep, lodStep, maxHeight);
  };
  const blockLightGrid = buildBlockLightGrid(lightSources, getCell, baseX, baseZ, lodStep, sizeX, sizeY, sizeZ);
    ...
    for (x[axis] = -1; x[axis] < dimensions[axis]; ) {
      ...
      for (x[vAxis] = 0; x[vAxis] < dimensions[vAxis]; x[vAxis]++) {
        for (x[uAxis] = 0; x[uAxis] < dimensions[uAxis]; x[uAxis]++) {
          ...
          const b = x[axis] < dimensions[axis] - 1 ? getCell(x[0] + q[0], x[1] + q[1], x[2] + q[2]) : BLOCK_ID_AIR;
