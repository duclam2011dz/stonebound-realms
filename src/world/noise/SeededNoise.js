import { createSeededPermutation } from "./createSeededPermutation.js";
...
function grad3Perlin(hash, x, y, z) {
  const h = hash & 15;
  const u = h < 8 ? x : y;
...
function dot2(grad, x, y) {
  return grad[0] * x + grad[1] * y;
}

function dot3(grad, x, y, z) {
...
export class SeededNoise {
  ...
  perlin3D(x, y, z) {
    ...
    const x4 = lerp(grad3Perlin(abb, xf, yf - 1, zf - 1), grad3Perlin(bbb, xf - 1, yf - 1, zf - 1), u);
  ...
  fractalPerlin2D(x, y, octaves = 4, persistence = 0.5, lacunarity = 2) {
    return this.fractal((fx, fy) => this.perlin2D(fx, fy), x, y, octaves, persistence, lacunarity);
  }

  fractalSimplex2D(x, y, octaves = 4, persistence = 0.5, lacunarity = 2) {
    return this.fractal((fx, fy) => this.simplex2D(fx, fy), x, y, octaves, persistence, lacunarity);
  }

  fractalPerlin3D(x, y, z, octaves = 4, persistence = 0.5, lacunarity = 2) {
    return this.fractal3D((fx, fy, fz) => this.perlin3D(fx, fy, fz), x, y, z, octaves, persistence, lacunarity);
  }

  fractalSimplex3D(x, y, z, octaves = 4, persistence = 0.5, lacunarity = 2) {
    return this.fractal3D((fx, fy, fz) => this.simplex3D(fx, fy, fz), x, y, z, octaves, persistence, lacunarity);
  }

  fractal(noiseFn, x, y, octaves, persistence, lacunarity) {
    let total = 0;
    let frequency = 1;
    let amplitude = 1;
    let maxValue = 0;
  ...
  fractal3D(noiseFn, x, y, z, octaves, persistence, lacunarity) {
    let total = 0;
    let frequency = 1;
    let amplitude = 1;
    let maxValue = 0;
